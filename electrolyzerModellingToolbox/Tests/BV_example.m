...
alpha = (0.3:0.1:0.7)';
jplus = permute(logspace(-1,1,7),[1 3 2]);
jperj0 = jplus.*(exp(alpha.*UactperfT)-exp((alpha-1).*UactperfT));


fig = figure('Name','Buttler-Volmer for variable alpha');
hold on;
for i = 1:numel(alpha)
    p(i) = plot(jperj0(i,:,4),UactperfT);
end
hold off;
yline(0)
...
%% Buttler-Volmer with variable j0

% UactperfT = -10:0.01:20;
% alpha = permute([0.3 0.5],[1 3 2]);
% j0 = logspace(-5,-4,5)';
% j = j0.*(exp(alpha.*UactperfT)-exp((alpha-1).*UactperfT));


fig = figure('Name','Buttler-Volmer for variable j0');
hold on;
for i = 1:numel(jplus)
    p(i) = plot(jperj0(4,:,i),UactperfT);
%     plot(j(i,:,2),UactperfT,'LineStyle','--','Color',p(i).Color)
end
hold off;
yline(0)
ylabel('$U_{\mathrm{act}}/fT$')
...
xlabel('$j/j_0$')
xlim([-40 40])
legendflex([p(1) p(2) p(3) p(4) p(5) p(6) p(7)],{'$10^{-1}$','$10^{-2/3}$','$10^{-1/3}$','$10^{0}$','$10^{1/3}$','$10^{2/3}$','$10^{1}$'},'xscale', 0.4,'anchor',{'se','se'},'buffer',[-10 10],'title','$j_{0+}$')
...
hold on;
p1 = plot(log(abs(jperj0(3,:,4))),UactperfT);
...
hold on;
p1 = plot(log(abs(jperj0(5,:,4))),UactperfT);
...
for i = 1:2:numel(alpha)
    j = i-(i-1)/2;
    p1(j) = plot(jperj0(i,:,4),UactperfT,'--','Color',colorOrder(j,:));
