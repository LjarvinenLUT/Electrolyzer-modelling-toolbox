    ...
    for j = 1:3
        ...
        for ii = 1:length(U1)
            ...
            elseif jmeastemp > 0.001
                U2 = Uohm(useParameters(3),jmeastemp); % Ohmic overpotential
                U3 = Ucon(useParameters(4),jmeastemp); % Concentration overpotential
                Umeastemp = Uocv+U1(ii)+U2+U3+useParameters(5); % "Measured" voltage
